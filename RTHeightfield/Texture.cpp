...
bool Texture::loadTexture( const void *pixels, int width, int height, GLenum internalFormat, GLenum dataFormat, GLenum dataType ) {
    if ( isLoaded() ) {
        closeTexture();
    }
...
bool Texture::loadTexture_16bitRGBA( const std::string &fileName ) {
    ...
    if ( success == IL_TRUE ) {
        ...
        if ( ( bpp / nch ) != 16 ) {
            std::cout << "Texture is not in RGBA 16 bpc format." << std::endl;
            return false;
        }
...
void Texture::downloadPixels() {
    if ( isLoaded() && !arePixelsLoaded() ) {
        ...
        _pixelsAW.reallocate( _textureWidth * _textureHeight * 4 );
        glGetTexImage( GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, _pixelsAW.getPointer() );
        unbindTexture();

        _pixelsLoaded = true;
    }
}
void Texture::downloadPixels( void* dataPtr, GLenum dataFormat, GLenum dataType ) {
    if ( isLoaded() ) {
        //if ( isLoaded() && !arePixelsLoaded() ) {
        bindTexture();
        glGetTexImage( GL_TEXTURE_2D, 0, dataFormat, dataType, dataPtr );
        unbindTexture();
    }
    //}
}
void Texture::uploadPixels( bool unloadPixels ) {
    if ( isLoaded() && arePixelsLoaded() ) {
        ...
        glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, _textureWidth, _textureHeight, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, _pixelsAW.getPointer() );
        unbindTexture();
...
Color4C *Texture::getPixels() {
    return (Color4C*)_pixelsAW.getPointer();
}
