...
bool RTHFShader::loadShader() {
    ...
    _ulNormalMapSampler = _shader.getUniformLocation( "uNormalMap" );
    ...
    _ulRealDims = _shader.getUniformLocation( "uRealDims" );
    ...
    glUniform1i( _ulNormalMapSampler, 3 );
...
void RTHFShader::closeShader() {
    if ( _shader.isCompiled() ) {
        ...
        glDeleteBuffers( 1, &_atomicRayCounterID );
        _atomicRayCounterID = 0;
    }
}
...
void RTHFShader::renderScene( const RTHFScene& scene ) {
    ...
    Vector3D realDimensions = Vector3D( scene.heightfield->getRealHeightmapDimensions(), scene.sceneHeight );

    //printArray( &realDimensions[ 0 ], 3 );

    //std::cout << "Heightmap Dims: "; printArray( &heightmapDimensions[ 0 ], 3 );
    apiut::setUniform3fv( _ulHeightmapDims, heightmapDimensions );
    apiut::setUniform3fv( _ulRealDims, realDimensions );
    ...
    scene.heightfield->getNormalMap()->bindTexture( 3 );
