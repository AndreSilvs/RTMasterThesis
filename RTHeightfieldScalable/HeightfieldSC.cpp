...
HeightfieldScalable::HeightfieldScalable() : _loadingTiles( false ) {

}
HeightfieldScalable::~HeightfieldScalable() {

}

bool HeightfieldScalable::initializeHeightmap( std::string hmapFileFormat, std::string hmapFileExtension,
    ...
    _normalW = _normalH = _hTileSize; // _hTileSize+1;
...
bool HeightfieldScalable::loadCompactFormat( const std::string& file ) {
...
bool HeightfieldScalable::loadCompactFormatA2( int tileX, int tileY, int orderTile ) {
    ...
    _tileCacheMap[ std::pair<int, int>( tileX, tileY ) ] = std::shared_ptr<TileCache>( new TileCache() );
    TileCache* tCache = getTileCache( tileX, tileY );
    tCache->tileX = tileX;
    tCache->tileY = tileY;
    tCache->assignedIndex = -1;
    tCache->error = false;
    tCache->loaded = true;
    tCache->heightTile.push_back( PixelArray<Color4C>() );
    ...
    while ( level <= _maxMipmapLevel ) {
        ...
        glTexSubImage2D( GL_TEXTURE_2D, level, lodWidth * sparseX, lodHeight * sparseY, lodWidth, lodHeight, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, mipmapData.getPixels() );
    ...
    ThreadSplitter::threadizeDual(
        ...
        for ( int x = xa; x < xb; ++x ) {
            for ( int y = ya; y < yb; ++y ) {
                Vector3D nUp = genNormal( &compactPixels, x, y, x, y + 1 );
                Vector3D nDown = genNormal( &compactPixels, x, y, x, y - 1 );
                Vector3D nLeft = genNormal( &compactPixels, x, y, x - 1, y );
                Vector3D nRight = genNormal( &compactPixels, x, y, x + 1, y );
    ...
    if ( getOpenGLError( "Error uploading normal texture data" ) ) {
        std::cout << "Error uploading normal texture data." << std::endl;
        return false;
    }
    ...
    while ( lodHeight != 1 && lodWidth != 1 ) {
        ...
        for ( int x = 0; x < lodWidth; ++x ) {
            for ( int y = 0; y < lodHeight; ++y ) {
                ...
                /*compactData.getPixel( x, y ).setColor(
                    compactPixels.getPixel( tx, ty ).red,
                    compactPixels.getPixel( tx + 1, ty ).green,
                    compactPixels.getPixel( tx, ty + 1 ).blue,
                    compactPixels.getPixel( tx + 1, ty + 1 ).alpha );*/
        ...
        // Create next level mipmap
        glTexSubImage2D( GL_TEXTURE_2D, level, lodWidth * sparseX, lodHeight * sparseY, lodWidth, lodHeight,
            GL_RGBA, GL_UNSIGNED_BYTE, compactData.getPixels() );
...
bool HeightfieldScalable::uploadTile( int tileX, int tileY ) {
    TileCache* tCache = getTileCache( tileX, tileY );
    if ( tCache == nullptr ) {
        return false;
    }
    ...
    for ( int i = 0; i < tCache->heightTile.size(); ++i ) {
        glTexSubImage2D( GL_TEXTURE_2D, i, tCache->heightTile[ i ].getWidth() * sparseX, tCache->heightTile[ i ].getHeight() * sparseY,
            tCache->heightTile[ i ].getWidth(), tCache->heightTile[ i ].getHeight(), GL_RGBA, GL_UNSIGNED_BYTE, tCache->heightTile[ i ].getPixels() );
    }
    _hfCompact.unbindTexture();
    tmr.stop();
    std::cout << "Stored HF tile: " << tmr.getTimeElapsed() / 1000.0 << "s" << std::endl;
    ...
    for ( int i = 0; i < tCache->mipmaps.size(); ++i ) {
        ...
        glTexSubImage2D( GL_TEXTURE_2D, i, tCache->mipmaps[i].getWidth() * sparseX, tCache->mipmaps[ i ].getHeight() * sparseY,
            tCache->mipmaps[ i ].getWidth(), tCache->mipmaps[ i ].getHeight(), GL_RGBA, GL_UNSIGNED_INT_8_8_8_8, tCache->mipmaps[i].getPixels() );
    }
...
bool HeightfieldScalable::loadPhotoTexture( const std::string &file ) {
    return _photoTexture.loadTexture( file, GL_COMPRESSED_RGBA_S3TC_DXT3_EXT, GL_RGBA, GL_UNSIGNED_BYTE );
}

TileCache* HeightfieldScalable::getTileCache( int tileX, int tileY ) {
...
void HeightfieldScalable::prepareToLoadTile( int tileX, int tileY ) {
    _tilesToLoad.push_back( std::pair< int, int>( tileX, tileY ) );
}
bool HeightfieldScalable::preLoadTile( int tileX, int tileY ) {
    // Determine which tile in the sparse texture is to be used
    ...
    tmr.start();
    tCache->heightTile.push_back( PixelArray<Color4C>() );
    if ( !Texture::loadPixelsFromImage( heightFile, tCache->heightTile[0] ) ) {
        //std::cout << "Tile loading error: Unable to read data from \"" << heightFile << "\"." << std::endl;
        tCache->error = true;
        return false;
    }
    ...
    for ( int i = 1; i <= _maxMipmapLevel; ++i ) {
        tCache->heightTile.push_back( PixelArray<Color4C>() );
        PixelArray<Color4C>& prevLevel = tCache->heightTile[ i - 1 ];
        PixelArray<Color4C>& nextLevel = tCache->heightTile[ i ];
    ...
    tCache->mipmaps.push_back( PixelArray<Color4C>() );
    PixelArray<Color4C>& topMip = tCache->mipmaps[ 0 ];
    ...
    ThreadSplitter::threadizeDual(
        ...
        for ( int x = xa; x < xb; ++x ) {
            for ( int y = ya; y < yb; ++y ) {
                Color4C sample = tCache->heightTile[0][ y * _hTileSize + x ];
                topMip.getPixel( x, y ).red =
                    (GLubyte)mathCalcs::max( mathCalcs::max( (int)sample.green, (int)sample.red ),
                        mathCalcs::max( (int)sample.blue, (int)sample.alpha ) );
            }
        }
    ...
    for ( int i = 1; i <= _maxMipmapLevel; ++i ) {
        tCache->mipmaps.push_back( PixelArray<Color4C>() );
        PixelArray<Color4C>& prevLevel = tCache->mipmaps[ i-1 ];
        PixelArray<Color4C>& nextLevel = tCache->mipmaps[ i ];
        int newW = prevLevel.getWidth() / 2;
        int newH = prevLevel.getHeight() / 2;
        nextLevel.createPixelArrayEmpty( newW, newH );
        ...
        for ( int x = 0; x < newW; ++x ) {
            for ( int y = 0; y < newH; ++y ) {
                ...
                nextLevel.getPixel( x, y ).red =
                    mathCalcs::max( mathCalcs::max( (int)prevLevel.getPixel( tx, ty ).red, (int)prevLevel.getPixel( tx + 1, ty ).red ),
                        mathCalcs::max( (int)prevLevel.getPixel( tx, ty + 1 ).red, (int)prevLevel.getPixel( tx + 1, ty + 1 ).red ) );
            }
        }
    }

    // Generate normals
     tCache->normalTile.createPixelArrayEmpty( _normalW, _normalH );

    ThreadSplitter::threadizeDual(
        [&]( int xa, int xb, int ya, int yb ) {
        for ( int x = xa; x < xb; ++x ) {
            for ( int y = ya; y < yb; ++y ) {
                Vector3D nUp = genNormal( &tCache->heightTile[0], x, y, x, y + 1 );
                Vector3D nDown = genNormal( &tCache->heightTile[ 0 ], x, y, x, y - 1 );
                Vector3D nLeft = genNormal( &tCache->heightTile[ 0 ], x, y, x - 1, y );
                Vector3D nRight = genNormal( &tCache->heightTile[ 0 ], x, y, x + 1, y );
...
bool HeightfieldScalable::isTilePreLoaded( int tileX, int tileY ) {
    TileCache* tCache = getTileCache( tileX, tileY );
    return ( tCache != nullptr ) && ( tCache->loaded );
}
/*bool HeightfieldScalable::loadDefaultPhotoTexture() {
...
/*Texture *HeightfieldScalable::getHeightmapTexture() {
    return &_hftex;
}*/

bool HeightfieldScalable::unloadTile( int x, int y ) {
...
void HeightfieldScalable::deleteTile( int tileX, int tileY ) {
    auto it = _tileCacheMap.find( std::pair<int, int>( tileX, tileY ) );
    if ( it != _tileCacheMap.end() ) {
        _tileCacheMap.erase( it );
    }
}
bool HeightfieldScalable::loadTile( int x, int y, int texTile ) {
    // Check if next tile is suitable
    if ( !loadCompactFormatA2( x, y, texTile ) ) {
        std::cout << "Failed to load tile: " << x << " , " << y << std::endl;
        return false;
    }
    return true;
}
void HeightfieldScalable::reloadRegion( int x, int y ) {
    for ( int x = 0; x < 3; ++x ) {
        for ( int y = 0; y < 3; ++y ) {
...
void HeightfieldScalable::recenterRegion( int nx, int ny ) {
    // Region data
    for ( int x = 0; x < 3; ++x ) {
        for ( int y = 0; y < 3; ++y ) {
...
void HeightfieldScalable::update( float dtime ) {
    ...
    if ( ( 0 <= userPosition.x && userPosition.x < getPatchSize()*getHeightmapWidth() ) && ( 0 <= userPosition.y || userPosition.y < getPatchSize()*getHeightmapHeight() ) ) {
        ...
        if ( isTilePreLoaded( uTileX, uTileY ) ) {
            ...
            Color4C patch = currentTile->heightTile[ 0 ].getPixel( (int)uX, (int)uY );
            ...
            float height = (float)mathCalcs::blerp( patch.alpha, patch.green, patch.blue, patch.red, interX, interY ) / 255.0f;
...
Vector3D HeightfieldScalable::genNormal( PixelArray<Color4C>* hmap, int ix, int iy, int tx, int ty ) {
...
int HeightfieldScalable::findAvailableTile() {
...
void HeightfieldScalable::printActiveRegion() {
    for ( int y = 0; y < 4; ++y ) {
        for ( int x = 0; x < 4; ++x ) {
...
void HeightfieldScalable::spawnWorkerThread() {
    // if bpc == 8
    std::thread wkThread{
        [&] {
            std::mutex mtx;
...
void HeightfieldScalable::closeHeightfield() {
