...
public:
    ...
    // Make this instance manage the OpenGL Texture ID passed as an argument.
    // Beware of duplicating texture handlers. Use at your own risk.
    bool assignTextureHandle( GLuint textureHandler );
    ...
    bool loadTexture( const Color4C *pixels, int width, int height, GLenum internalFormat, GLenum dataFormat, GLenum dataType );
    ...
    static bool loadPixelsFromImage16b_RGBA( const std::string &fileName, PixelArray< Color4C_16b > &pixels );
    static bool loadCompressedPixelsFromImage( const std::string &fileName, ArrayWrapper< GLubyte > &pixels );
    ...
    template <typename T>
    T* getPixels() {
        return reinterpret_cast<T*>( _pixels.getPixels() );
    }
